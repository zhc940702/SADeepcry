...
class Protein_Crystallization(nn.Module):
    ...
    def forward(self, protein_sequence, fingerprint, device):
        ...
        vae_out, encode = self.vae(fingerprint.to(device))

        fingerprint = F.relu(self.protein_finger_bn(self.protein_finger_layer(encode)), inplace=True)
        ...
        # print(self.total)
        return p_feature, vae_out

class Protein_Crystallization_zqc(nn.Module):
    def __init__(self, protein_sequence_dim, protein_finger_dim, hid_embed_dim, attention_number, dropout,
                 embed_dim=128,conv_num=32, protein_kernel = [3,5,7],protein_MAX_LENGH = 1000,
                 **base_config_TextCNN):
        super(Protein_Crystallization_zqc, self).__init__()

        self.embed_dim = embed_dim
        self.conv = conv_num
        self.protein_MAX_LENGH = protein_MAX_LENGH
        self.protein_kernel = protein_kernel
        self.protein_embed = nn.Embedding(22, self.embed_dim, padding_idx=0)
        self.Protein_CNNs = nn.Sequential(
            nn.Conv1d(in_channels=self.embed_dim, out_channels=self.conv, kernel_size=self.protein_kernel[0]),
            nn.ReLU(),
    ...
    def forward(self, protein_sequence, fingerprint, device):
        ...
        proteinembed = self.protein_embed(protein_sequence)
        p_feature = self.attention(proteinembed)
        p_feature = p_feature.view(p_feature.shape[0], -1)
        p_feature = F.relu(self.protein_sequence_bn(self.protein_sequence_layer(p_feature)), inplace=True)
        p_feature = F.dropout(p_feature, training=self.training, p=self.dropout)
        p_feature = self.protein_sequence_layer_1(p_feature)
